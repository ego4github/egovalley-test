    ...
    let HZRecorder = function (stream, config) {
        ...
        let audioData = {
            ...
            encodeWAV: function () {
                ...
                // 写入采样数据
                if (sampleBits === 8) {
                    for (let i = 0, len = bytes.length; i < len; i++, offset++) {
                    ...
                    for (let i = 0, len = bytes.length; i < len; i++, offset += 2) {
                        let s = Math.max(-1, Math.min(1, bytes[i]));
        ...
        // 上传
        this.upload = function (sessionId, url, callback) {
            let fd = new FormData();
            fd.append('flag', "true");
            fd.append('id', "998");
            ...
            /*$.ajax({
                type: 'POST',
                url: commonObj.basePath + '/asr/asrPart',
                data: fd,
                dataType : 'JSON',
                cache : false, // 不缓存
